...
# 1. Collects configuration (NODE_ID, domain, workers, password)
# 2. Generates secrets and wallet keys
# 3. Automatically starts all services
# 4. Optionally shows live block building stats
...
print_help() {
    cat << EOF
Usage: ./setup-galleon-testnet.sh [OPTIONS]

Interactive setup script for IGRA Galleon Testnet deployment.
...
For manual setup, see: doc/quick-setup-galleon-testnet.md
EOF
}
...
main() {
    ...
    echo "========================================"
    echo "  IGRA Galleon Testnet Setup Script"
    echo "========================================"
    echo
    ...
    if [[ -f .env ]]; then
        log "Existing .env file found."
        if prompt_confirm "Do you want to backup and replace it?" "y"; then
            backup_file=".env.backup.$(date +%Y%m%d_%H%M%S)"
            cp .env "$backup_file"
            log "Backed up to $backup_file"
        else
            log "Keeping existing .env file. Some values may be overwritten."
        fi
    fi
    ...
    if [[ ! -f .env ]]; then
        if [[ ! -f .env.galleon-testnet.example ]]; then
            die "Template file .env.galleon-testnet.example not found in $PROJECT_DIR"
        fi
    ...
    echo "The GTN- prefix will be added automatically."
    ...
    NODE_ID="GTN-${NODE_NAME}"
    if [[ ${#NODE_ID} -gt 64 ]]; then
        die "NODE_ID too long. Maximum 64 characters allowed (including GTN- prefix)."
    fi
  ...
  - Ubuntu/Debian: sudo apt install expect"
        ...
        for i in "${MISSING_WALLETS[@]}"; do
            keyfile="keys/keys.kaswallet-${i}.json"
            log "Generating wallet $i..."
            # Use expect to handle interactive password prompts
            # The kaswallet-create tool requires TTY for password input
            ...
            if ! WALLET_PASS="$WALLET_PASSWORD" expect -c '
                log_user 0
                set password $env(WALLET_PASS)
                spawn docker run --rm -it --user '"$(id -u):$(id -g)"' -v "'"$PROJECT_DIR"'/keys:/keys" --entrypoint /app/kaswallet-create \
                    '"$KASWALLET_IMAGE"' --testnet -k /keys/keys.kaswallet-'"${i}"'.json
                expect "password:"
                send "$password\r"
                expect "password"
                send "$password\r"
                expect eof
            ' 2>&1 | tee -a /tmp/wallet-gen.log > /dev/null; then
                die "Failed to generate wallet $i. Check Docker and try again."
            fi
    ...
    if [[ "$RUNNING_CONTAINERS" -gt 0 ]]; then
        ...
        if prompt_confirm "Do you want to restart all services with new configuration?" "y"; then
            log "Stopping existing services..."
            docker compose down 2>/dev/null || true
        else
            log "Keeping existing services. New configuration may not take effect until restart."
        fi
    fi
    ...
    echo "  2. After IBD sync completes (IDB: 100%):"
    ...
    if prompt_confirm "Would you like to see live sync progress and block building stats?" "y"; then
        ...
        trap 'echo; log "Stats viewer stopped."; exit 0' INT
